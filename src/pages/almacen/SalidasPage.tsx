...
import { Search, Plus, Package, ArrowUp, Trash2 } from 'lucide-react'
...
export default function SalidasPage() {
  ...
  const mutation = useMutation({
    mutationFn: (dto: Parameters<typeof almacenApi.salidaBatch>[0]) => almacenApi.salidaBatch(dto),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map(m => (
                  <tr key={m.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-2 text-right font-semibold text-danger-500">-{m.cantidad}</td>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Registrar salida de mercancía" size="lg">
